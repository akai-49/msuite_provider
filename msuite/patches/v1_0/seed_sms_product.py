"""
Seed the SMS product + plans in-place.

Creates or reseeds the SMS product and SMS-* plans from the seeds definition.
"""
import frappe
from frappe.utils import now

from msuite.seeds.sms import (
...
def execute():
    product_name = frappe.db.get_value(
        "MSuite Product", {"product_code": "SMS"}, "name"
    )
    if not product_name:
        product = frappe.new_doc("MSuite Product")
        product.product_code = "SMS"
        product.product_name = "SMS"
        product.description = "SMS marketing, transactional messaging, and automation platform"
        product.is_active = 1
    ...
    for plan in SMS_PLANS:
        ...
        _reseed_plan_features(
            plan["plan_code"],
            SMS_PLAN_FEATURES[plan["plan_code"]],
            feature_name_by_key,
        )
...
def _reseed_product_features(product_name: str) -> None:
    ...
    for feature in SMS_PRODUCT_FEATURES:
