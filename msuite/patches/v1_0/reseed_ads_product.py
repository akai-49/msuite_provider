"""
Reseed the Ads product + plans in-place.

Wipes old feature rows on the Ads product and on every ADS-* plan,
and builds them from the seeds definition.
"""
import frappe
from frappe.utils import now

from msuite.seeds.ads import (
...
def execute():
    product_name = frappe.db.get_value(
        "MSuite Product", {"product_code": "ADS"}, "name"
    )
    if not product_name:
        product = frappe.new_doc("MSuite Product")
        product.product_code = "ADS"
        product.product_name = "Ads"
        product.description = "Digital advertising management platform"
        product.is_active = 1
    ...
    for plan in ADS_PLANS:
        ...
        _reseed_plan_features(
            plan["plan_code"],
            ADS_PLAN_FEATURES[plan["plan_code"]],
            feature_name_by_key,
        )
...
def _reseed_product_features(product_name: str) -> None:
    ...
    for feature in ADS_PRODUCT_FEATURES:
