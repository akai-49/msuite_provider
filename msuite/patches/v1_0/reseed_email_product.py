"""
Reseed the Email product + plans in-place.

Wipes old feature rows on the Email product and on every EMAIL-* plan,
and builds them from the seeds definition.
"""
import frappe
from frappe.utils import now

from msuite.seeds.email import (
...
def execute():
    product_name = frappe.db.get_value(
        "MSuite Product", {"product_code": "EMAIL"}, "name"
    )
    if not product_name:
        product = frappe.new_doc("MSuite Product")
        product.product_code = "EMAIL"
        product.product_name = "Email"
        product.description = "Email marketing and automation platform"
        product.is_active = 1
    ...
    for plan in EMAIL_PLANS:
        ...
        _reseed_plan_features(
            plan["plan_code"],
            EMAIL_PLAN_FEATURES[plan["plan_code"]],
            feature_name_by_key,
        )
...
def _reseed_product_features(product_name: str) -> None:
    ...
    for feature in EMAIL_PRODUCT_FEATURES:
