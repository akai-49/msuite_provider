"""
Reseed the WhatsApp product + plans in-place.
"""
import frappe
from frappe.utils import now

from msuite.seeds.whatsapp import (
    WA_PRODUCT_FEATURES,
    WA_PLANS,
    WA_PLAN_FEATURES,
)
...
def execute():
    product_name = frappe.db.get_value(
        "MSuite Product", {"product_code": "WA"}, "name"
    )
    if not product_name:
    ...
    for plan in WA_PLANS:
        ...
        _reseed_plan_features(
            plan["plan_code"],
            WA_PLAN_FEATURES[plan["plan_code"]],
            feature_name_by_key,
        )
...
def _reseed_product_features(product_name: str) -> None:
    ...
    for feature in WA_PRODUCT_FEATURES:
...
def _ensure_plan_exists(plan_cfg: dict, product_name: str) -> None:
    ...
    Plan metadata (name, code, tier, item, rate) is only set on first create;
    re-runs never mutate existing plans. Features are handled separately.
    """
